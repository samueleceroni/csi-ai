...
void update_utility_cell(double utilities[MAX_X+1][MAX_Y+1][N_UTILITIES], int x, int y, int n_utility){
	...
	switch(max_utility_index_nearby){
		case GO_UP:
			max_utility_nearby = utilities[x][y+1][n_utility-1];
			break;
		case GO_RIGHT:
			max_utility_nearby = utilities[x+1][y][n_utility-1];
			break;
		case GO_DOWN:
			max_utility_nearby = utilities[x][y-1][n_utility-1];
			break;
	...
	utilities[x][y][n_utility] = utilities[x][y][n_utility-1] + (LEARNING_RATE) * ((REWARD) + (GAMMA) * (max_utility_nearby - utilities[x][y][n_utility-1]));
}
